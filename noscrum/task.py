...
def update_task(
    task_id, task, story_id, estimate, status, actual, deadline, sprint_id, recurring
):
    """
...
@router.get("/create/{story_id}", response_class=HTMLResponse)
def get_create_form(story_id: int, is_asc: bool = False):
    """
    ...
    return templates.TemplateResponse(
        "task/create.html", {"story": story, "asc": is_asc}
    )
...
def api_create(story_id: int, task: Task, epic_id: int = 0):
    ...
    if error is None:
        task = create_task(
            task.task, story_id, task.estimate, task.deadline, task.sprint_id
        )
