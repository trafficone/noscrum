        ...
        for task in schedule_tasks:
            task_dict = dict(
                zip(
                    keys,
                    [
                        str(task[k]) if isinstance(task[k], date) else task[k]
                        for k in keys
                    ],
                )
            )
...
def create():
    ...
    if request.method == "POST":
        ...
        if not start_date:
            ...
            get_sprint_by_date(middle_date=start_date) is not None
            or get_sprint_by_date(middle_date=end_date) is not None
        ):
...
def active():
    ...
    if is_json:
        return json.dumps(
            {"Success": True, "sprint_id": sprint_id, "sprint": dict(current_sprint)}
        )
