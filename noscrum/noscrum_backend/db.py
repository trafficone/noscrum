...
class DictableModel:
    """
    Interface class - any Model which can be converted
    ...
    def to_dict(self):
        ...
        for column in self.__table__.columns:  # pylint: disable=no-member
            ...
            if isinstance(value, Hashable) and not isinstance(value, date):
                pass
...
class UserPreference(get_db().Model, DictableModel):
    """
...
class Task(get_db().Model, DictableModel):
    """
...
class Tag(get_db().Model, DictableModel):
    """
...
class Story(get_db().Model, DictableModel):
    """
...
class Epic(get_db().Model, DictableModel):
    """
    ...
    def to_dict(self):
        ...
        for column in self.__table__.columns:  # pylint: disable=no-member
            ...
            if isinstance(value, Hashable):
                ...
                value = repr(value)
            ret[key] = value
        ret["stories"] = [x.to_dict() for x in self.stories]
        return ret


class Sprint(get_db().Model, DictableModel):
    """
...
class ScheduleTask(get_db().Model):
    """
