...
class noscrumEpicTest(noscrumTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.test_epic = main.db.Epic(epic='TEST EPIC', color='red', deadline=date(
    ...
    def test_get_epic(self, current_user):
        ...
        epic = main.epic.get_epic(self.epic.id)
        self.assertIsNotNone(epic, 'Epic Not None')
    ...
    def test_get_epic_by_name(self, current_user):
        ...
        current_user.return_value = user
        self.create_epic_once()
        epic = main.epic.get_epic_by_name(self.test_epic.epic)
